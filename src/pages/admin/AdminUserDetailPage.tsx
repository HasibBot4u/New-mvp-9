...
import { ArrowLeft, User, Clock, MessageSquare, Monitor, Calendar } from "lucide-react";
...
export default function AdminUserDetailPage() {
  ...
  return (
    <div className="p-4 md:p-8 flex flex-col gap-6 bg-background text-foreground min-h-full rounded-tl-xl border-l border-t border-border">
      ...
      {profile.loading ? <LoadingSkeleton /> : profile.error ? <ErrorUI error={profile.error} onRetry={() => fetchData(`/api/admin/users/${userId}/profile`, setProfile, 'profile')} /> : profile.data && (
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col">
        <div className="overflow-x-auto pb-2 mb-4 shrink-0 mt-2">
        ...
        <div className="flex-1 overflow-y-auto">
          <TabsContent value="overview" className="m-0 focus:outline-none flex flex-col gap-6">
            {stats.loading ? <LoadingSkeleton /> : stats.error ? <ErrorUI error={stats.error} onRetry={() => fetchData(`/api/admin/users/${userId}/stats`, setStats, 'stats')} /> : (
            ...
            {activity.loading || watchHistory.loading ? <LoadingSkeleton /> : (
              <div className="border border-border p-6 rounded-xl overflow-x-auto bg-surface shadow-sm">
                <h3 className="font-semibold mb-6 flex items-center text-foreground tracking-tight"><Calendar className="w-4 h-4 mr-2 text-foreground/60" /> Activity Contributions (Last 12 Weeks)</h3>
          ...
          <TabsContent value="watch" className="m-0 focus:outline-none">
            {watchHistory.loading ? <LoadingSkeleton /> : watchHistory.error ? <ErrorUI error={watchHistory.error} onRetry={() => fetchData(`/api/admin/users/${userId}/watch-history`, setWatchHistory, 'history')} /> : (
